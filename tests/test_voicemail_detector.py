...
class TestVoicemailDetectorFallback(unittest.IsolatedAsyncioTestCase):
    """Without a confident answer, silence decides with the best answer so far."""
    ...
    async def test_a_confident_conversation_waits_for_the_caller_to_stop(self):
        classifier = _FakeClassifier(("conversation", 0.99), ("voicemail", 0.98))
        detector = VoicemailDetector(
            classifier=classifier, voicemail_response_delay=VOICEMAIL_DELAY
        )
        ...
        # but the greeting goes on and turns out to be a voicemail.
        await run_test(
            detector,
            frames_to_send=[
                UserStartedSpeakingFrame(),
        ...
        self.assertEqual(fired["voicemail"], [detector])
