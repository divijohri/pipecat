...
class _FakeApi:
    ...
    def classifier(self) -> JevClassifier:
        return JevClassifier(client=JevClient(api_key="k", http_client=self.http()))
...
def _questions(post: dict) -> dict:
    """The questions a request carries, by name."""
    return post["body"]["questions"]


def _is_goal(post: dict) -> bool:
    """Whether a run request is the goal's: only a turn's carries the reply."""
    return "latest_bot_reply" not in post["body"]["state"]


def _judge(responses, explainer=None, **kwargs) -> tuple[JevEvalJudge, _FakeApi]:
    api = _FakeApi(responses)
    judge = JevEvalJudge(classifier=api.classifier(), explainer=explainer, **kwargs)
    return judge, api
...
class TestJevEvaluate(unittest.IsolatedAsyncioTestCase):
    async def test_the_latest_reply_is_judged_after_the_spoken_conversation(self):
        ...
        question = _questions(request)["verdict"]
    ...
    async def test_without_continue_a_reply_is_judged_yes_or_no(self):
        ...
        judge = JevEvalJudge(classifier=api.classifier(), allow_continue=False)
        ...
        self.assertEqual(set(_questions(api.posts[0])["verdict"]["criteria"]), {"yes", "no"})
    ...
    async def test_an_answer_of_the_wrong_type_is_a_failed_request(self):
        judge, _ = _judge([_ok({"answer": _noul(0.9)})] * 2)
        judge.add_assistant_message("anything")
        verdict = await judge.evaluate("anything")
        self.assertEqual(verdict.verdict, "no")
        self.assertEqual(verdict.reason, "judge call failed")
...
class TestJevEvaluateRun(unittest.IsolatedAsyncioTestCase):
    ...
    def _answer(goal: float, turns: dict[str, float | dict], failing: str | None = None):
        ...
        def respond(body: dict) -> tuple[int, dict]:
            ...
            if reply is None:
                return (503, {}) if failing == "goal" else _ok({"answer": _noul(goal)})
            if reply == failing:
                return 503, {}
            return _ok({name: _turn(turns[reply]) for name in body["questions"]})
    ...
    async def test_the_goal_is_asked_over_the_whole_conversation(self):
        ...
        goal = next(r["body"] for r in api.posts if _is_goal(r))
        self.assertEqual(
    ...
    async def test_each_turn_is_judged_after_only_the_conversation_before_it(self):
        ...
        turns = [r["body"] for r in api.posts if not _is_goal(r)]
    ...
    async def test_a_turn_the_criterion_does_not_apply_to_passes(self):
        ...
        question = next(_questions(r)["apology"] for r in api.posts if not _is_goal(r))
...
class TestJevConfig(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        # A judge built from a config makes its own connection; give it a mock one,
        # so these tests reach no network and need no HTTP/2 support installed.
        patcher = patch.object(
            JevEvalJudge,
            "_new_classifier",
            lambda judge, api_key, model, base_url, timeout: _FakeApi([]).classifier(),
        )
        patcher.start()
    ...
    async def test_close_leaves_a_classifier_it_was_given_alone(self):
        api = _FakeApi([])
        http = api.http()
        judge = JevEvalJudge(
            classifier=JevClassifier(client=JevClient(api_key="k", http_client=http))
        )
        await judge.close()
...
def _timeout() -> httpx.ReadTimeout:
    return httpx.ReadTimeout("stalled")


class _OwnConnectionJudge(JevEvalJudge):
    """A judge that makes its own classifier, over a mock connection to one API."""

    def __init__(self, api: _FakeApi, **kwargs):
        self.api = api
        super().__init__(api_key="k", **kwargs)

    def _new_classifier(self, api_key, model, base_url, timeout) -> JevClassifier:
        return self.api.classifier()
...
class TestJevConnection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_close_cleans_up_a_classifier_the_judge_made(self):
        api = _FakeApi([])
        judge = _OwnConnectionJudge(api)
        await judge.close()
        self.assertTrue(judge._classifier.client._http.is_closed)
